...
options.registry.oe_cat_slider = options.Class.extend({
  start: function (editMode) {
    ...
    if (!editMode) {
      self.$el
        .find(".oe_cat_slider")
        .on("click", $.bind(self.cat_slider, self));
    }
  },
  ...
  cat_slider: function (type, value) {
    ...
    if ((type != undefined && type == false) || type == undefined) {
      self.$modal = $(
        renderToElement("theme_scita.scita_dynamic_category_slider")
      );
...
options.registry.theme_scita_product_slider = options.Class.extend({
  start: function (editMode) {
    ...
    if (!editMode) {
      self.$el
        .find(".oe_prod_slider")
        .on("click", $.bind(self.prod_slider, self));
    }
  },
  ...
  prod_slider: function (type, value) {
    ...
    if ((type != undefined && type == false) || type == undefined) {
      self.$modal = $(
        renderToElement("theme_scita.scita_dynamic_product_slider")
      );
      self.$modal.appendTo("body");
      self.$modal.modal("show");
      var $slider_type = self.$modal.find("#slider_type"),
...
options.registry.fashion_multi_cat_custom_snippet = options.Class.extend({
  start: function (editMode) {
    ...
    if (!editMode) {
      self.$el
        .find(".fashion_multi_category_slider")
        .on("click", $.bind(self.multi_category_slider, self));
    }
  },
  ...
  multi_category_slider: function (type, value) {
    ...
    if ((type != undefined && type == false) || type == undefined) {
      self.$modal = $(
        renderToElement("theme_scita.multi_product_custom_slider_block")
      );
      self.$modal.appendTo("body");
      self.$modal.modal("show");
      var $slider_type = self.$modal.find("#slider_type"),
      ...
      $snippnet_submit.on("click", function () {
        ...
        self.$target.attr(
          ...
          "multi-cat-myowl" + $slider_type.val()
        );
        if ($("select#slider_type").find(":selected").text()) {
          var type = "";
          type = _t($("select#slider_type").find(":selected").text());
...
options.registry.brands_box_slider_4 = options.Class.extend({
  start: function (editMode) {
    ...
    if (!editMode) {
      self.$el
        .find(".box_brand_slider")
        .on("click", $.bind(self.box_brand_slider, self));
    }
  },
...
options.registry.it_prod_brands = options.Class.extend({
  start: function (editMode) {
    ...
    if (!editMode) {
      self.$el
        .find(".it_brand_slider")
        .on("click", $.bind(self.brand_it_slider, self));
    }
  },
...
options.registry.theme_scita_blog_custom_snippet = options.Class.extend({
  start: function (editMode) {
    ...
    if (!editMode) {
      self.$el
        .find(".scita_blog_slider")
        .on("click", $.bind(self.theme_scita_blog_slider, self));
    }
  },
  onBuilt: function () {
    var self = this;
  ...
  theme_scita_blog_slider: function (type, value) {
    ...
    if ((type != undefined && type == false) || type == undefined) {
      ...
      rpc("/theme_scita/blog_get_options", {}).then(function (res) {
        ...
        $.each(res, function (y) {
          $("select[id='blog_slider_type']").append(
            $("<option>", {
              value: res[y]["id"],
              text: res[y]["name"],
            })
...
options.registry.product_category_img_slider_config = options.Class.extend({
  start: function (editMode) {
    ...
    this.$target.removeClass("hidden");
    this.$target
      .find(".multi_product_and_category_slider .owl-carousel")
      .empty();
    if (!editMode) {
      self.$el
        .find(".multi_product_and_category_slider")
        .on("click", $.bind(self.multi_category_slider, self));
    }
  },
  ...
  multi_category_slider: function (type, value) {
    ...
    if ((type != undefined && type == false) || type == undefined) {
      self.$modal = $(
        renderToElement("theme_scita.scita_product_category_img_slider_config")
      );
      self.$modal.appendTo("body");
      self.$modal.modal("show");
      var $slider_type = self.$modal.find("#slider_type"),
...
options.registry.sct_product_snippet_1 = options.Class.extend({
  ...
  multi_category_slider: function (type, value) {
    ...
    if ((type != undefined && type == false) || type == undefined) {
      self.$modal = $(
        renderToElement(
          "theme_scita.scita_dynamic_product_snippet_configuration"
        )
      );
      self.$modal.appendTo("body");
      self.$modal.modal("show");
...
options.registry.dynamic_video_banner = options.Class.extend({
  start: function (editMode) {
    ...
    if (!editMode) {
      self.$el
        .find(".dynamic_video_banner")
        .on("click", $.bind(self.dynamic_video_banner, self));
    }
  },
...
options.registry.animaion_effect = options.Class.extend({
  start: function () {
    ...
    if (this.$target.parent().attr("class") != undefined) {
      var parentclassName = this.$target
        .parent()
        .attr("class")
        .match("sct_img_effect(.)");
      if (parentclassName) {
  ...
  _computeWidgetState(methodName, params) {
    this.$target
      .parent()
      .removeClass(
        "sct_img_effect1 sct_img_effect2 sct_img_effect3 sct_img_effect4 sct_img_effect5"
      );
...
options.registry.theme_scita_trending_products = options.Class.extend({
  start: function (editMode) {
    ...
    if (!editMode) {
      self.$el
        .find(".theme_scita_trending_products")
        .on("click", $.bind(self.trending_products, self));
    }
  },
  ...
  trending_products: function (type, value) {
    ...
    if ((type != undefined && type == false) || type == undefined) {
      self.$modal = $(
        renderToElement("theme_scita.scita_trending_products_modal")
      );
      self.$modal.appendTo("body");
      self.$modal.modal("show");
      var $slider_type = self.$modal.find("#slider_type"),
        $trending_products_category_delete = self.$modal.find("#cancel"),
        $trending_category_data_submit = self.$modal.find(
          "#trending_category_data_submit"
        );
      rpc("/theme_scita/trending_category_get_options", {}).then(function (
        res
      ) {
        $('#slider_type option[value!="0"]').remove();
        $.each(res, function (y) {
