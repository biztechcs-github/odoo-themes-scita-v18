...
animation.registry.oe_cat_slider = animation.Class.extend({
  ...
  start: function () {
    ...
    if (this.editableMode) {
      var $cate_slider = $("#wrapwrap").find(
        "#theme_scita_custom_category_slider"
      );
...
animation.registry.theme_scita_product_slider = animation.Class.extend({
  ...
  start: function () {
    var self = this;
    if (this.editableMode) {
      var $prod_slider = $("#wrapwrap").find(
        "#theme_scita_custom_product_slider"
      );
...
animation.registry.fashion_multi_cat_custom_snippet = animation.Class.extend({
  ...
  start: function () {
    var self = this;
    if (this.editableMode) {
      var $multi_cat_slider = $("#wrapwrap").find(
                                                ...
                                                <div class="row our-categories">\
                                                    <div class="col-md-12">\
                                                        <div class="title-block">\
                                                            <h4 id="snippet-title" class="section-title style1"><span>' +
              multi_cat_name +
              "</span></h4>\
                                                        </div>\
                                                    </div>\
      ...
      rpc("/fashion/fashion_product_multi_get_dynamic_slider", {
        ...
        if (data) {
          ...
          $(document).ajaxComplete(function () {
            setTimeout(function () {
              var divWidth = $(
                ".fashion_featured_product_1 .cs-product .pwp-img a"
              ).width();
              if (divWidth == 0) {
                divWidth = $(
                  ".retail_featured_product_1 .cs-product .pwp-img a:visible"
                ).width();
              }
              $(".fashion_featured_product_1 .cs-product .pwp-img a").height(
                divWidth
              );
            }, 100);
          });
...
animation.registry.third_client_slider_snippet = animation.Class.extend({
  ...
  start: function () {
    ...
    if (!this.editableMode) {
      rpc("/theme_scita/third_get_clients_dynamically_slider", {}).then(
        function (data) {
          if (data) {
            self.$target.empty();
            self.$target.append(data);
...
animation.registry.cat_slider_4 = animation.Class.extend({
  ...
  start: function () {
    ...
    if (this.editableMode) {
      var $cate_slider = $("#wrapwrap").find(
        "#theme_scita_custom_category_slider_4"
      );
      var cat_name = _t("Category Slider");
...
animation.registry.product_category_img_slider_config = animation.Class.extend({
  ...
  start: function () {
    ...
    if (this.editableMode) {
      var $multi_cat_slider = $("#wrapwrap").find(
        ".multi_product_and_category_slider"
      );
    ...
    if (!this.editableMode) {
      ...
      rpc("/product_column_five", {
        ...
        if (data) {
          ...
          setTimeout(function () {
            var divWidth = $(
              ".sct_product_snippet_1 .cs-product .pwp-img a"
            ).width();
...
animation.registry.dynamic_top_dealers = animation.Class.extend({
  ...
  start: function () {
    ...
    if (!this.editableMode) {
      $.get("/theme_scita/top_dealers", {}).then(function (data) {
        self.$target.empty();
        self.$target.append(data);
      });
    }
  ...
  start: function () {
    ...
    if (!this.editableMode) {
      ...
      $.get("/theme_scita/trending_products_categories", {
        ...
        self.$target.append(data);
        const categoryId =
          $(".trend_prod_tab")[0].getAttribute("data-category-id");
      ...
      $(document).on("click", ".trend_prod_tab", function (ev) {
        ...
        $.get("/theme_scita/get_trending_prducts", {
          ...
          self.$target.append(data);
          $(".theme_scita_trending_products").removeClass("o_hidden");
          for (let ele of $("#trending_products_categories").find(
            "li.trend_prod_tab > a"
          )) {
            if (
            ...
            if (
              ele.parentElement.getAttribute("data-category-id") ==
              ev.currentTarget.getAttribute("data-category-id")
            ) {
